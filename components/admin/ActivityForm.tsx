...
export default function ActivityForm({ isOpen, onClose, onSuccess, initialData }: ActivityFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setAlertState({
        isOpen: true,
        title: "Kegiatan Disimpan",
        message: "Artikel kegiatan berhasil diterbitkan ke website.",
        type: "success"
      });
      ...
      setAlertState({
        ...
        message: "Terjadi kesalahan saat mengupload data atau gambar.",
        type: "error"
      });
      console.error(error);
    } finally {
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="bg-slate-50 p-4 rounded-xl space-y-4 border border-slate-100">
              ...
              <div className="space-y-1">
                  ...
                  {initialData?.mainImage && (
                      <p className="text-xs text-green-600 mt-1">Sudah ada foto cover.</p>
                  )}
              </div>

              {/* Gallery */}
              <div className="space-y-1 pt-2 border-t border-slate-200">
                  <label className="text-xs font-medium text-slate-500">Galeri Tambahan</label>
                  <input 
                      type="file" 
                      accept="image/*"
                      multiple
                  ...
                  {initialData?.gallery && initialData.gallery.length > 0 && (
                       <div className="text-xs text-slate-400 mt-1 flex items-center gap-1">
                          <UploadCloud size={12} />
                          {initialData.gallery.length} foto tersimpan.
                       </div>
                  )}
              </div>
          </div>
          ...
          <div className="flex items-center gap-4 pt-2">
              <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  />
                  <span className="text-sm text-slate-700">Jadikan Featured</span>
              </label>
